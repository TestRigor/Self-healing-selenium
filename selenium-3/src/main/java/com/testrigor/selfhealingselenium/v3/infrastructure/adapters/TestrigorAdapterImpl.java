package com.testrigor.selfhealingselenium.v3.infrastructure.adapters;
...
import com.testrigor.selfhealingselenium.commons.application.utils.JsonHelpers;
...
public class TestrigorAdapterImpl implements TestrigorAdapter {
	...
	public List<String> getNodesXPaths(String pageSource) {
		val client = client();
		RequestBody body = RequestBody.create(JSON, JsonHelpers.serializeJson(new LocatorsIdentifiersCommand(pageSource)));
		...
		try (Response response = client.newCall(request).execute()) {
			...
			if (response.isSuccessful()) {
				return JsonHelpers.deserializeJson(response.body().string(), XPATH_TYPE_REF);
			}
	...
	public void saveLocator(Action action, List<NodeInformation> pageNodes, String pageSource, String testCaseName) {
		...
		val client = client();
		RequestBody body = RequestBody.create(JSON, JsonHelpers.serializeJson(command));
		Request request = new Request.Builder()
	...
	public Locator getHealedLocator(Locator locator, List<NodeInformation> pageNodes, String pageSource, String testCaseName) {
		...
		try (Response response = client.newCall(request).execute()) {
			...
			if (response.isSuccessful()) {
				val healedLocatorResponse = JsonHelpers.deserializeJson(response.body().string(), HealedLocatorResponse.class);
				return new Locator(
