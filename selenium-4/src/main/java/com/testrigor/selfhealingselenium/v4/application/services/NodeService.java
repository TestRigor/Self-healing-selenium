package com.testrigor.selfhealingselenium.v4.application.services;
...
import com.testrigor.selfhealingselenium.commons.application.parser.NodeInformationDeserializer;
...
public class NodeService {
	...
	public List<NodeInformation> getBatchNodeInformation(@NonNull Collection<String> xPathCollection) {
		...
		if (jsResultRaw instanceof String) {
			jsResult = JsonHelpers.deserializeJson((String) jsResultRaw, NODE_INFO_TYPE_REF);
		} else {
			jsResult = (List<Map<String, Object>>) jsResultRaw;
		}

		for (val entry : jsResult) {
			val info = NodeInformationDeserializer.deserializeNodeInformation(entry);
			if (info.isDummy()) {
				continue;
			}
			results.push(info);
